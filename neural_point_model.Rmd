...
tmp <- 
  inner_join(x = tmp, y = players_keep_atp, by = "name")
...
nn_mod_df <- 
  ...
  mutate(ServerHand = case_when(
    servingplayer %in% tmp$winner_name ~ tmp$winner_hand,
    servingplayer %in% tmp$loser_name ~ tmp$loser_hand
  ))
...
```{r}
serve_nn <- neuralnet(ServeWidth ~ ServeIndicator + point_score + ServeDepth + ServeSide,
                      data = nn_train_df,
                      linear.output = FALSE,
                      err.fct = 'ce',
                      likelihood = TRUE,
