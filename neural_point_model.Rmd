...
# sample for training data
nn_train_df <- 
  nn_mod_df[sample(1:nrow(nn_mod_df), 12000, replace = FALSE), ]
...
```{r}
# fit model
serve_nn <- neuralnet(serve_width ~ serve_number + server_up + serve_side + Handedness + returnheight + Fed + 
                      Nadal + Novak + Milos + Nishikori + Stan + Thiem + Murray + Isner + Querrey + Edmund +
...
nn_test_df %>%
  group_by(serve_width) %>%
  summarise(count = n())
# model predicts 2087 / 4526 = 0.46

# see how many nn predicts correct
table(nn_test_df$serve_width, apply(new_pred, 1, which.max))
# nn predicts (1075 + 1258) / 4526 = 0.515
```


```{r}
# compare predictions with observed (first version of nn)
table(nn_test_df$serve_width, apply(serve_pr, 1, which.max))
# sensitivity (W) = (( + ) / ( +  +  + )) = 
# specificity (W) = (( + ) / ( +  +  + )) = 
# sensitivity (C) = (( + ) / ( +  +  + )) = 
# specificity (C) = (( + ) / ( +  +  + )) = 
