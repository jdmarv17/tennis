...
nn_mod_df <-
  nn_mod_df %>%
  unite("ServePlacement", c(ServeWidth, ServeDepth))
```
...
```{r}
# fit model
serve_nn <- neuralnet(ServePlacement ~ serve_number + server_up + serve_side + Handedness,
                      data = nn_train_df,
                      linear.output = FALSE,
                      hidden = c(3,2))
plot(serve_nn)
...
```{r}
# predictions
serve_pr <- neuralnet::compute(serve_nn, nn_test_df)

```


```{r}
# similar pattern between righties and lefties
# only lefty is nadal though
ggplot(nn_mod_df, aes(x = ServePlacement)) +
  geom_bar() +
  facet_wrap(~ hand + ServeSide)

# pretty similar pattern between sides too
ggplot(nn_mod_df, aes(x =ServePlacement)) +
  geom_bar() +
