...
```{r}
# fit model
serve_nn <- neuralnet(serve_width ~ serve_number + server_up + serve_side + Handedness + Fed + 
                        Nadal + Novak + Milos + Nishikori + Stan + Thiem + Murray,
                      data = nn_train_df,
                      linear.output = FALSE,
                      hidden = c(6,3))
plot(serve_nn)
...
```{r}
# predictions
serve_pr <- predict(serve_nn, nn_test_df)
