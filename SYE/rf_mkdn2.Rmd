...
# get predictions
preds <- predict(rf_allscore_real, test2)
test2 <- 
  test2 %>%
  ungroup %>%
...
test2$PointNumber <- as.numeric(test2$PointNumber)
...
test2 %>% 
  filter(p1 == "Donald Young" & p2 == "Gael Monfils" & slam == "usopen" & year == "2017") %>%
  ggplot(., aes(y = predictions, x = PointNumber)) +
  geom_line()
