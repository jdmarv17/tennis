...
atp_practice_df<- atp_df %>% group_by(match_id) %>%
  slice(1) %>%
  ungroup() %>%
...
atp_practice2 <- atp_df %>% filter(player1 %in% atp_practice_df$name &
...
# atp_mod1 has random slopes and intercepts for servingplayer and returningplayer
atp_mod1 <- lmer(serverwin ~ importance + (1 + servingplayer | servingplayer) + 
                   (1 + returningplayer| returningplayer), data = atp_small_df)
summary(atp_mod1)
...
summary(atp_mod1)
## there is a warning that said the original was "singular"....this isn't
## super important but a warning that a matrix is singular is bad...
## we can talk about this more when we chat
## the random slope model should allow a different slope between importance
## and server win for each player. Therefore, importance needs to be on the 
## Left of the | 
...
atp_mod3 <- lmer(serverwin ~ importance + (1 + importance | servingplayer) +
                   (1 + importance | slam), data = atp_small_df)
summary(atp_mod3)
## probably want to change slam to be a fixed effect. There are only 4 slams ever
## so that may make more sense. 
## Can also think about it another way: the model needs to estimate 
## $\hat{\sigma}_{slam}$, and, with just 4 slams, estimating the variance
## of 4 things is __really__ difficult.
## If you want slopes to differ by slam, can add the usual fixed
## effect interaction


# atp_mod4 has random intercept and slope for slam
atp_mod4 <- lmer(serverwin ~ importance + (1 + importance | slam), data = atp_small_df)
summary(atp_mod4)
## same here....it's probably too difficult to estimate a variance
## for the slam intercept and a variance for the slam slope with just
## 4 slams.
```
...
ggplot(atp_small_df, aes(x = importance, y = serverwin, color = servingplayer)) +
  ...
  theme_bw() 
```
*Im still surprised how most players trend downwards as importance goes up*
*if there is someway to figure out why this is the case it could be an interesting result I think*
...
ggplot(atp_small_df, aes(x = importance, y = serverwin, colour = servingplayer)) +
  ...
  theme_bw() 
```
*taking out returningplayer didnt change intercepts much*
