...
```{r}
newdat <- data.frame(servingplayer = c("Global", "Global"), importance = c(min(small_df$importance,
  na.rm = TRUE), max(small_df$importance, na.rm = TRUE)))
predict(mod_random, newdata= newdat, re.form = ~0)


ggplot(small_df, aes(x = importance, y = serverwin, colour = servingplayer)) +
  geom_point() +
  geom_line(data = newdat, aes(y = predict(mod_random, newdata = newdat, re.form = ~0)), size = 2)  + ##the line above this should be changed to be a global fit.
