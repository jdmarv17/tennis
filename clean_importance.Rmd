...
atp_importance_new <- rbind(atp_importance5, new_obs)
```
...
```{r}
testdf <- data.frame(point = c("0-0", "0-0", "0-15", "0-0", "15-15", "15-15"),
  group = c("A", "A", "A", "B", "B", "B"), 
  importance = c(0.1, NA, 0.12, 0.13, NA, 0.17))
testdf
testdf %>% group_by(point, group) %>%
...
tmp_importance <-
  atp_importance5 %>%
  filter(serve_set == 2 & return_set == 2) %>%
  separate(game_score, into = c("score1", "score2"), sep = "-",
    convert = TRUE) %>%
  filter(score1 >= 5 & score2 >= 5) %>%
  filter(score1 < 6 | score2 < 6) %>%
